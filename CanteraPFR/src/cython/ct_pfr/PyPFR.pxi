...
cdef class PyPFR:
    ...
    2. Add interface to user viscosity function.
    3. Make htc a function of gas speed.
    4. Allow vector of absolute tolerances.
    5. Parse wall profile from string with SymPy.
    ...
    def manage_solution(self, Lr, dx, saveas, outfreq=100, overwrite=False):
        ...
        overwrite : bool, optional
            If `True`, allow results to be overwritten. Default is `False`.
        ...
        for i, x in enumerate(saveat[1:]):
            ...
            if self.sol.solve(x) == -99:
                raise Exception('Failure during integration')
